...
import {removeTask, updateTack} from "../../tasksReducer";
...
export const Task = React.memo(({task, todolist,...props}: TaskPropsType) => {
    ...
    const changeStatus = useCallback(function (e: ChangeEvent<HTMLInputElement>) {
        ...
        dispatch(updateTack({taskId:task.id , todolistId:todolist.id, domainModel: {status: newIsDoneValue} } ));
    }, [task.id, todolist.id]);

    const changeTaskTitle = useCallback(function (newTitle: string) {
        dispatch(updateTack({taskId:task.id , todolistId:todolist.id, domainModel: {title: newTitle} } ));
    }, [task.id, todolist.id]);
